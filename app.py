...
def dashboard():
    ...
    return render_template('index.html', plot_html = safe_html, plot_html1= safe1_html, plot_html2=safe2_html, plot_html3=safe3_html)
...
def eventos():
    ...
    plt1_html = graph.generate_plot("SELECT tipo_de_evento, condicion, COUNT(no_folio) as No_Accidentes FROM tablename GROUP BY 1,2", plot_type='bar', x_axis='tipo_de_evento', y_axis='No_Accidentes', className = "evento", xName = "Tipo de evento", yName = "Número de accidentes", Titulo = "Condición por evento", rotation = 90, hue="condicion")
    safe1_html = Markup(plt1_html)
    plt2_html = graph.generate_plot("SELECT year(fecha_evento) as Fecha, COUNT(no_folio) as NUM FROM tableName GROUP BY 1 ORDER BY 1", plot_type='line_plot', x_axis='Fecha', y_axis='NUM', className = "fechas", xName = "Fecha", yName = "Número de accidentes", Titulo = "Axidentes por año", rotation = 90)
    safe2_html = Markup(plt2_html)
    plt3_html = graph.generate_plot("SELECT genero, COUNT(no_folio) as NUM_G FROM tableName GROUP BY 1", plot_type='pie', x_axis='genero', y_axis='NUM_G', className = "fechas", xName = "", yName = "", Titulo = "Género", rotation = 90)
    safe3_html = Markup(plt3_html)
    return render_template('index.html', plot_html = safe_html, plot_html1= safe1_html, plot_html2=safe2_html, plot_html3=safe3_html)
